...
public class Application1 {
    public static void main(String[] args) {
        ...
        // 사용자 정의 자료형인 클래스를 사용하기 위해서는 new연산자로 heap영역에 할당을 해야한다.
        // 객체를 생성하게 되면 클래스에 정의한 필드와 메소드 대로 객체가 생성 된다.
        Member member = new Member();

        // 필드에 접근하기 위해 레퍼런스변수명, 필드명으로 접근한다.
        // '.'은 참조 연산자라고 하는데, 레퍼런스 변수가 참조하는 주소로 접근한다는 의미를 가진다.
        // heap영역은 기본적으로 JVM 초기값으로 초기화된다.
//        System.out.println("member.id = " + member.id);
//        System.out.println("member.name = " + member.name);
//        System.out.println("mamber.pwd = " + member.pwd);
